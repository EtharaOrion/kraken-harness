...
Each task in this dataset was produced by the [`{pipeline}` pipeline](https://github.com/huggingface/Repo2RLEnv/blob/main/docs/pipelines/{pipeline}.md). The pipeline mines real merged pull requests / commits from the source repo(s), applies quality filters, strips information-leakage from the instruction text, and emits a [Harbor](https://github.com/Ethara-Ai/harbor)-shaped task directory with the gold patch as the oracle.
    ...
    *,
    ...
    if not shutil.which("harbor"):
        raise RuntimeError(
            ...
            "Install with `source .env && uv tool install git+https://${GITHUB_TOKEN}@github.com/Ethara-Ai/harbor`, "
            "then re-run. Requires GITHUB_TOKEN with repo access (private repo)."
        )
