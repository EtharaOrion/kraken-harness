#!/usr/bin/env python3
"""Stage 7 Test Runner — runs all test files, generates per-file logs, and a consolidated report.

Usage:
    python tests/stage7/runner.py
    python tests/stage7/runner.py -k test_extract
    python tests/stage7/runner.py --parallel
    python tests/stage7/runner.py --verbose
"""
...
PROJECT_ROOT = STAGE_DIR.parent.parent


def discover_test_files() -> list[Path]:
    return sorted(STAGE_DIR.glob("test_*.py"))
...
def generate_consolidated_report(results: list[dict], duration_total: float) -> str:
    lines: list[str] = []
    lines.append("=" * 90)
    lines.append("  STAGE 7 — WORKLOAD GENERATION TEST SUITE — CONSOLIDATED REPORT")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 7 test suite runner")
    ...
    if not test_files:
        print("No test files found in", STAGE_DIR)
        sys.exit(1)
