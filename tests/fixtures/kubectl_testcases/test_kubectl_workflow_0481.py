def test_workflow_0481_job_lifecycle(cli, kubectl_bin, tmp_path):
    ...
    r_get = cli("get", "job", "wf-job-0481", "-n", "default")
    assert r_get.returncode == 0, r_get.stderr
    r_desc = cli("describe", "job", "wf-job-0481", "-n", "default")
    assert r_desc.returncode == 0, r_desc.stderr
    r_del = cli("delete", "job", "wf-job-0481", "-n", "default")
