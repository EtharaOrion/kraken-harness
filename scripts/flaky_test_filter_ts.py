...
"""TypeScript analog of ``flaky_test_filter.py`` (Paper App. C.5, Stage V).

Reads N correctness-run logs for each instance, parses test statuses via
``log_parsers_ts``, drops tests whose status is not stable across runs.
...
def _read_status_map(repo: str, log_path: Path) -> dict[str, str]:
    if not log_path.exists():
        return {}
    parser = MAP_REPO_TO_PARSER_TS.get(repo, parse_log_ts_best_effort)
    try:
