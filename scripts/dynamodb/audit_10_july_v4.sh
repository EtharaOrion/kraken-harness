#!/usr/bin/env bash
#
# 10-july-v4 audit: run harbor oracle + nop against each task under
# raiden/10-july-v4/{uuid}/. Reject any task where oracle reward != 1.0
# or nop reward != 0.0.
#
# Per-task outputs:
...
#
# Batch outputs at raiden/10-july-v4/:
#   harbor_runs.log      per-task ORACLE + NOP panels, ALL COMPLETE footer
#   audit_verdict.tsv    uuid \t oracle_reward \t nop_reward \t verdict
#
# UUIDs are discovered dynamically (any dir under V4_ROOT that isn't
# harbor_runs.log / audit_verdict.tsv is treated as a task).
#
# Env (optional):
#   V4_ROOT        default: /Users/anshkataria/Desktop/7-July/raiden/10-july-v4
#   JOBS_ROOT      default: /tmp/audit-jobs-10-july-v4
#   HARBOR_BIN     default: /Users/anshkataria/Desktop/7-July/raiden/harbor/.venv/bin/harbor
#   START_AT       1..N to resume mid-run

set -euo pipefail

V4_ROOT="${V4_ROOT:-/Users/anshkataria/Desktop/7-July/raiden/10-july-v4}"
JOBS_ROOT="${JOBS_ROOT:-/tmp/audit-jobs-10-july-v4}"
HARBOR_BIN="${HARBOR_BIN:-/Users/anshkataria/Desktop/7-July/raiden/harbor/.venv/bin/harbor}"
START_AT="${START_AT:-1}"

[ -d "$V4_ROOT" ] || { echo "V4_ROOT not found: $V4_ROOT" >&2; exit 1; }
[ -x "$HARBOR_BIN" ] || { echo "harbor CLI not executable: $HARBOR_BIN" >&2; exit 1; }
docker info > /dev/null 2>&1 || { echo "docker daemon not running" >&2; exit 1; }

# Dynamic UUID discovery: any directory directly under V4_ROOT that has task.toml.
UUIDS=()
...
done < <(find "$V4_ROOT" -mindepth 1 -maxdepth 1 -type d | sort)

TOTAL=${#UUIDS[@]}
[ "$TOTAL" -gt 0 ] || { echo "no task dirs discovered under $V4_ROOT" >&2; exit 1; }

HARBOR_RUNS_LOG="$V4_ROOT/harbor_runs.log"
VERDICT_TSV="$V4_ROOT/audit_verdict.tsv"
