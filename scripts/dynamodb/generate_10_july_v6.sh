#!/usr/bin/env bash
#
# 10-july-v6 DDB batch: generate the single "hardest" DDB task using the
...
# Env (optional):
#   OUT_ROOT             parent dir (default: ./datasets/v6)
...
for i in "${!SUBSETS[@]}"; do
  ...
  subset="${SUBSETS[$i]}"
  out="$OUT_ROOT/ddb-v6-t${n}"
  echo
  echo "===================================================================="
  echo "[10-july-v6] task ${n}/${#SUBSETS[@]} -> $out"
  echo "  subset: $subset"
...
echo
echo "[10-july-v6] batch complete. Task dirs under: $OUT_ROOT/ddb-v6-t{${START_AT}..${END_AT}}/"
