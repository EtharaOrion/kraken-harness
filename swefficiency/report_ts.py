...
"""TypeScript analog of ``report.py``.
...
``MAP_REPO_TO_PARSER_TS`` instead of the Python parser map.
"""
...
def evaluate_instance_ts(
    instance: dict,
    gold_run: Path,
    pred_run: Path,
    use_correctness_files: bool = True,
) -> Dict:
    """TypeScript version of ``report.evaluate_instance``.

    Behaviorally identical to the Python pipeline except the per-test status
    parsing falls back to ``MAP_REPO_TO_PARSER_TS`` (with a
    ``parse_log_ts_best_effort`` fallback for unknown repos).
    """
    ...
    if not use_correctness_files:
        ...
        parser = MAP_REPO_TO_PARSER_TS.get(instance["repo"], parse_log_ts_best_effort)
...
def generate_report_ts(
    gold_run: Path,
    pred_run: Path,
    output_dir: Path,
    num_workers: int = 4,
    dataset_name: str = "swefficiency/swefficiency-ts",
) -> Tuple[pd.DataFrame, Dict, Path, Path]:
    """TypeScript analog of ``report.generate_report``."""
    ...
    worker = partial(evaluate_instance_ts, gold_run=gold_run, pred_run=pred_run)
    with multiprocessing.Pool(num_workers) as pool:
        results = list(
            tqdm(
                pool.imap(worker, ds, chunksize=1),
                total=len(ds),
                desc="Evaluating ts instances",
            )
        )

    results_df = pd.DataFrame(results)

    csv_path = output_dir / f"eval_report_ts_{report_name}.csv"
    results_df.to_csv(csv_path, index=False)

    breakdown = compute_performance_breakdown(results_df)
    breakdown["report"] = csv_path.name
    breakdown["language"] = "ts"
    json_path = output_dir / f"eval_report_ts_{report_name}.json"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate TypeScript evaluation report.")
    ...
    _, breakdown, _, _ = generate_report_ts(
