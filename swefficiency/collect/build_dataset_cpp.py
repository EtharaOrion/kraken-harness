...
"""Build C++ dataset rows from collected pull requests.
...
* Every emitted row is tagged ``language='cpp'`` so downstream consumers
...
def create_instance(pr: dict, repo: str, language: str = "cpp") -> dict:
    ...
    try:
        ...
        write_to_dlq(
            "build_dataset_cpp_fetch_failed.jsonl",
            {
                "instance_id": instance_id,
                "repo": repo,
                "stage": "extract_patches_cpp",
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--repo", required=True, help="GitHub org/repo (e.g. fmtlib/fmt)")
...
def main(argv: Optional[list] = None) -> int:
    ...
    logger.info(
        "build_dataset_cpp done: completed=%d perf_kept=%d fetch_failed=%d",
