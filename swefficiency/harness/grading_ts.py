...
"""TypeScript grading: mirrors ``grading.py`` but routes logs to TS parsers.

Without this fork, ``_ParserMapWithFallback`` in ``grading.py`` silently
falls back to ``parse_log_pytest`` for every TypeScript instance and emits
an empty test-status map (every test treated as missing -> F2P=0).
"""
...
from swefficiency.harness.test_spec_ts import TestSpecTs


# Vitest reports per-test outcomes as one of the lowercase strings below
# (see vitest's TaskResultPack: "passed" | "failed" | "skipped" | "todo" |
# "only"). We normalize them to swefficiency's canonical TestStatus values
# so the downstream report logic (test_passed / test_failed) keeps working
# unchanged. The cpp sibling normalizes its own runner outputs identically.
_VITEST_STATUS_MAP: dict[str, str] = {
    "passed": TestStatus.PASSED.value,
    "failed": TestStatus.FAILED.value,
    "skipped": TestStatus.SKIPPED.value,
    # `todo` is vitest's "this test is planned but not implemented yet"
    # marker -- treat as SKIPPED, not FAILED, to match how pytest's xfail
    # and the cpp sibling's disabled-test markers are graded.
    "todo": TestStatus.SKIPPED.value,
    # `only` indicates a test is filtered-in via `.only`; if it reaches
    # grading without a passed/failed outcome it means the runner short-
    # circuited -- treat as ERROR so it doesn't silently count as passed.
    "only": TestStatus.ERROR.value,
}


def normalize_vitest_status(status: str) -> str:
    """Map a vitest wire status to a canonical :class:`TestStatus` value.

    Unknown statuses normalize to ``ERROR`` rather than raising, matching
    how :func:`parse_log_ts_best_effort` handles malformed parser output.
    """
    return _VITEST_STATUS_MAP.get(status.lower(), TestStatus.ERROR.value)
...
def get_logs_eval_ts(
    log_fp: str,
    repo: Optional[str] = None,
) -> tuple[dict[str, str], bool]:
    """Parse TS test output. Returns ``(status_map, patch_applied_ok)``.

    If ``repo`` is passed, route via :data:`MAP_REPO_TO_PARSER_TS` directly.
    ...
    Unknown repos route to :func:`parse_log_ts_best_effort` instead of
    ...
    log_parser = MAP_REPO_TO_PARSER_TS.get(repo, parse_log_ts_best_effort)
...
def get_eval_report_ts(
    ...
    """TypeScript analog of ``get_eval_report``."""
    ...
    # Treat an empty-string patch the same as a missing one. run_evaluation_ts
    ...
    eval_sm, found = get_logs_eval_ts(log_path, repo=repo or test_spec.repo)
    if not found:
        return report_map
