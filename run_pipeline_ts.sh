...
###############################################################################
# run_pipeline_ts.sh — End-to-end SWE-fficiency TypeScript pipeline
#
# Mirrors run_pipeline.sh (Python) but calls the *_ts variants for stages that
...
#
# Phase 1 (this script): vertical slice on lodash/lodash and the 5 other Tier-1
# TypeScript repos. Coverage and flaky_filter are SOFT-SKIPPED (warn + continue)
# for Phase 1 — they require run_validation_ts.py which is Phase 2. The script
# will not crash if those tools are missing.
#
# Usage:
#   ./run_pipeline_ts.sh [OPTIONS]
#
# Options:
#   --repo OWNER/NAME       Target repo (default: lodash/lodash)
#   --repos-file PATH       File with TypeScript repos, one per line
...
if [[ -n "$REPOS_FILE" ]]; then
    REPO_SLUG="multi"
    REPO_OWNER="multi"
    [[ -z "$RUN_ID" ]] && RUN_ID="ts_multi_$(date +%Y%m%d_%H%M%S)"
else
    REPO_SLUG="${REPO##*/}"
    REPO_OWNER="${REPO%%/*}"
    [[ -z "$RUN_ID" ]] && RUN_ID="ts_${REPO_SLUG}_$(date +%Y%m%d_%H%M%S)"
fi

ARTIFACTS_DIR="$SCRIPT_DIR/artifacts_ts"
...
WORKLOAD_DIR="$SCRIPT_DIR/logs/workload_generation_ts/$RUN_ID"
EVAL_DIR="$SCRIPT_DIR/logs/run_evaluation_ts/$RUN_ID"
REPORT_DIR="$SCRIPT_DIR/eval_reports_ts"
...
# ─── Stage gating ────────────────────────────────────────────────────────────
ORDERED_STAGES_TS=(discover scrape perf_filter versioning detect_specs coverage flaky_filter workload eval significance_filter pred_eval report inference)

validate_stage_name() {
    local name="$1"
    for s in "${ORDERED_STAGES_TS[@]}"; do
        [[ "$s" == "$name" ]] && return 0
    done
    echo "ERROR: Unknown stage '$name'. Valid: ${ORDERED_STAGES_TS[*]}"
    exit 1
}
...
should_run_stage() {
    ...
    if [[ -n "$START_FROM" || -n "$STOP_AFTER" ]]; then
        ...
        for _s in "${ORDERED_STAGES_TS[@]}"; do
...
check_prereqs() {
    ...
    # Soft check: TypeScript toolchain (do not fail — node:20-bookworm-slim base
    # image carries node/npm; autodetected pnpm/yarn/bun are repo-driven).
    local ts_missing=()
    for tool in node npm pnpm yarn bun tsc vitest; do
        command -v "$tool" >/dev/null 2>&1 || ts_missing+=("$tool")
    done
    if [[ ${#ts_missing[@]} -gt 0 ]]; then
        echo "INFO: Host missing TypeScript tools (${ts_missing[*]}); Docker base image will provide them."
    fi
}

###############################################################################
# STAGE 0: Discover TypeScript repos from GitHub (license-filtered, no hardcoded set)
###############################################################################
stage_discover() {
    log "STAGE 0: Discovering open-source TypeScript repos from GitHub"
    ...
    if [[ -z "$DISCOVERY_OUTPUT" ]]; then
        DISCOVERY_OUTPUT="$ARTIFACTS_DIR/discovered_ts_repos.txt"
    fi

    step "Searching GitHub for TypeScript repos with allowed licenses..."
    ...
    run_cmd python3 -m swefficiency.collect.discover_repos_ts \
    ...
    if [[ ! -f "$DISCOVERY_OUTPUT" ]]; then
        echo "ERROR: discover_repos_ts produced no output at $DISCOVERY_OUTPUT"
        exit 1
    fi
...
###############################################################################
# STAGE 1-3: Scrape + Build Dataset (TypeScript)
###############################################################################
stage_scrape() {
    if [[ -n "$REPOS_FILE" ]]; then
        log "STAGE 1-3: Scraping PRs (ts) from $REPOS_FILE"
    else
        log "STAGE 1-3: Scraping PRs (ts) from $REPO"
    fi
    ...
    step "Scraping with get_tasks_pipeline_ts..."
    run_cmd python3 -m swefficiency.collect.get_tasks_pipeline_ts "${pulls_args[@]}"
...
###############################################################################
# STAGE 5: Versioning (TypeScript)
###############################################################################
stage_versioning() {
    log "STAGE 5: Versioning (ts)"
    ensure_dir "$VERSIONED_DIR"
    echo "  Cache:    cache_ts.db (namespace: NS_VERSION_TS)"

    step "Detecting versions via get_versions_ts..."
    run_cmd python3 -m swefficiency.versioning.get_versions_ts \
...
###############################################################################
# STAGE 6: Auto-detect Repo Specs (TypeScript)
###############################################################################
stage_detect_specs() {
    log "STAGE 6: Auto-detecting ts repo specs"
    ensure_dir "$ENRICHED_DIR"
    ENRICHED_FILE="$ENRICHED_DIR/${REPO_SLUG}_enriched.jsonl"
    echo "  Cache:    cache_ts.db (namespace: NS_REPO_SPECS_TS)"

    step "Detecting package manager, node engine, tsconfig.json, deps..."
    run_cmd python3 scripts/detect_repo_specs_ts.py \
...
###############################################################################
stage_coverage() {
    log "STAGE 4.5: Coverage Detection (ts) — Phase 1 stub"
    echo "  Coverage requires run_validation_ts.py (Phase 2). Skipping."
...
###############################################################################
stage_flaky_filter() {
    log "STAGE 4.6: Flaky Test Detection (ts) — Phase 1 stub"
    echo "  Flaky filter requires N=$FLAKY_RUNS correctness runs via run_validation_ts.py (Phase 2)."
    echo "  Dataset passes through unchanged."
    return 0
}

###############################################################################
# STAGE 7: Workload Generation (LLM, TypeScript)
###############################################################################
stage_workload() {
    log "STAGE 7: Generating ts workloads via LLM"
    ...
    # default to ON for pipeline runs so DLQ at workload_uncompilable_ts.jsonl
    # is actually reachable. Honour any user override.
    export SWEFF_VALIDATE_TS_WORKLOAD="${SWEFF_VALIDATE_TS_WORKLOAD:-1}"
    export WORKLOAD_MODEL_TS="${WORKLOAD_MODEL_TS:-}"

    echo "  Workload file: /tmp/workload.bench.ts (vitest bench / tinybench)"
    echo "  Validation:    SWEFF_VALIDATE_TS_WORKLOAD=$SWEFF_VALIDATE_TS_WORKLOAD"

    step "Generating vitest bench workloads..."
    run_cmd python3 -m swefficiency.workload.run_synthetic_generation_ts \
        --dataset_name "$ENRICHED_FILE" \
        --run_id "$RUN_ID" \
        --max_workers "$MAX_WORKERS"

    local wl_output
    wl_output=$(find "logs/workload_generation_ts/$RUN_ID" -name "workload_generation_ts.json" -type f 2>/dev/null | head -1)
    if [[ -z "$wl_output" ]]; then
        wl_output=$(find "logs/workload_generation_ts/$RUN_ID" -name "*.json" -type f 2>/dev/null | head -1)
    fi

    ensure_dir "$FINAL_DIR"
    FINAL_DATASET="$FINAL_DIR/${REPO_SLUG}-ts-dataset.jsonl"
...
###############################################################################
# STAGE 8: Docker Build + Gold Evaluation (TypeScript)
###############################################################################
stage_eval() {
    log "STAGE 8: Build ts images + Gold Evaluation"
    ...
    echo "  Base image: sweb.base.ts:latest (node:20-bookworm-slim)"
    echo "  Env tag:    sweb.env.ts.<hash>:latest"
    echo "  Eval tag:   sweb.eval.ts.<id>:latest"

    # Stage 8a: Build ts images.
    step "Building ts Docker images via build_and_validate_images_ts..."
    local build_args=(
        --dataset "$dataset_to_use"
        --max-workers "$MAX_WORKERS"
    )
    $MULTIARCH && build_args+=(--build-multiarch)
    run_cmd python3 scripts/build_and_validate_images_ts.py "${build_args[@]}" || {
    ...
    # We construct an empty-patch predictions JSONL to drive run_evaluation_ts.
    ensure_dir "$EVAL_DIR"
...
"
    ...
    run_cmd python3 -m swefficiency.harness.run_evaluation_ts \
...
stage_significance_filter() {
    ...
    ensure_dir "$FINAL_DIR"
    SIGNIFICANT_DATASET="$FINAL_DIR/${REPO_SLUG}-ts-significant.jsonl"
...
###############################################################################
# STAGE 8.5: Prediction Evaluation (gold-as-pred for ts)
###############################################################################
stage_pred_eval() {
    log "STAGE 8.5: Prediction Evaluation (ts, gold-as-pred)"
    ...
    ensure_dir "$FINAL_DIR"
    local pred_file="$FINAL_DIR/${REPO_SLUG}-ts-gold-predictions.jsonl"
...
###############################################################################
# STAGE 9: Report Generation (ts)
###############################################################################
stage_report() {
    log "STAGE 9: Report Generation (ts)"
    if $DRY_RUN; then
        echo "  [DRY-RUN] skipped"
        return
    fi
    ...
    PRED_DIR="$SCRIPT_DIR/logs/run_evaluation_ts/${RUN_ID}_pred/gold_as_pred"
    [[ ! -d "$PRED_DIR" ]] && PRED_DIR=$(find "$SCRIPT_DIR/logs/run_evaluation_ts/${RUN_ID}_pred" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | head -1 || true)

    local dataset_to_use="${FINAL_DATASET:-${ENRICHED_FILE:-$DATASET}}"
    [[ -n "$DATASET" ]] && dataset_to_use="$DATASET"

    if [[ -d "$GOLD_DIR" && -d "$PRED_DIR" ]]; then
        step "Generating ts comparison report..."
        run_cmd python3 -m swefficiency.report_ts \
            --gold-run "$GOLD_DIR" \
            --pred-run "$PRED_DIR" \
        ...
        echo ""
        echo "  Reports:"
        ls -la "$REPORT_DIR"/eval_report_ts_*.* 2>/dev/null || echo "  (no report files)"
        if [[ -f "$REPORT_DIR/eval_report_ts_gold_as_pred.json" ]]; then
            step "Report Summary:"
            cat "$REPORT_DIR/eval_report_ts_gold_as_pred.json"
        fi
    else
...
###############################################################################
# STAGE 10: Inference — Phase 2 stub for ts
###############################################################################
stage_inference() {
    log "STAGE 10: Inference (ts) — Phase 2 stub"
    echo "  TypeScript OpenHands inference deferred to Phase 2."
    return 0
}
...
###############################################################################
print_summary() {
    log "TypeScript Pipeline Complete!"
    echo ""
    echo "  Run ID:        $RUN_ID"
    echo "  Repo:          $REPO"
...
main() {
    if [[ -n "$REPOS_FILE" ]]; then
        log "SWE-fficiency TypeScript Pipeline — multi-repo (from $REPOS_FILE)"
    else
        log "SWE-fficiency TypeScript Pipeline — $REPO"
    fi
    echo "  Run ID:      $RUN_ID"
