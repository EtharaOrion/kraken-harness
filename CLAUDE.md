...
7. **`code_instruct` cli_app mode uses MinIO + the `minio` Python SDK** — zero `boto3`/`botocore`/`moto` fingerprint in shipped artifacts (Dockerfile, `pip list`, tests, oracle, conftest). MinIO + mc binaries are SHA256-pinned into the base image (~120 MB); a module-scoped subprocess fixture launches `minio server` on a free port (~1s startup); per-test reset is iterate-and-delete via the SDK (~50-200ms). The `cli` fixture sets `MINIO_*` env (primary, read by `submission/main.py`) and `AWS_*` env (defensive, for any aws-cli call in test bodies). NB: generic AWS mode (`code_instruct.py` `aws_mode=True` branch — DynamoDB/Kinesis/etc.) and the `[bedrock]` optional extra remain on moto+boto3; only cli_app was migrated. See `plans/minio_migration_plan.md` for the migration record.
