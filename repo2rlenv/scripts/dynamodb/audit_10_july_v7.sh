#!/usr/bin/env bash
#
# 10-july-v7 audit: run harbor oracle + nop against each task under
# raiden/10-july-v7/{uuid}/. Reject any task where oracle reward != 1.0
# or nop reward != 0.0.
#
# Per-task outputs:
...
#
# Batch outputs at raiden/10-july-v7/:
#   harbor_runs.log      per-task ORACLE + NOP panels, ALL COMPLETE footer
#   audit_verdict.tsv    uuid \t oracle_reward \t nop_reward \t verdict
#
# UUIDs are discovered dynamically (any dir under V7_ROOT that has task.toml).
#
# Env (optional):
#   V7_ROOT        default: /Users/anshkataria/Desktop/7-July/raiden/10-july-v7
#   JOBS_ROOT      default: /tmp/audit-jobs-10-july-v7
#   HARBOR_BIN     default: /Users/anshkataria/Desktop/7-July/raiden/harbor/.venv/bin/harbor
#   START_AT       1..N to resume mid-run

set -euo pipefail

V7_ROOT="${V7_ROOT:-/Users/anshkataria/Desktop/7-July/raiden/10-july-v7}"
JOBS_ROOT="${JOBS_ROOT:-/tmp/audit-jobs-10-july-v7}"
HARBOR_BIN="${HARBOR_BIN:-/Users/anshkataria/Desktop/7-July/raiden/harbor/.venv/bin/harbor}"
START_AT="${START_AT:-1}"

[ -d "$V7_ROOT" ] || { echo "V7_ROOT not found: $V7_ROOT" >&2; exit 1; }
[ -x "$HARBOR_BIN" ] || { echo "harbor CLI not executable: $HARBOR_BIN" >&2; exit 1; }
docker info > /dev/null 2>&1 || { echo "docker daemon not running" >&2; exit 1; }

# Dynamic UUID discovery: any directory directly under V7_ROOT that has task.toml.
UUIDS=()
...
done < <(find "$V7_ROOT" -mindepth 1 -maxdepth 1 -type d | sort)

TOTAL=${#UUIDS[@]}
[ "$TOTAL" -gt 0 ] || { echo "no task dirs discovered under $V7_ROOT" >&2; exit 1; }

HARBOR_RUNS_LOG="$V7_ROOT/harbor_runs.log"
VERDICT_TSV="$V7_ROOT/audit_verdict.tsv"
