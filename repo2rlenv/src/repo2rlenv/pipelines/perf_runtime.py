...
# The environment is a prebuilt, content-addressed image. A rollout pulls it rather
# than rebuilding the repository, and the digest is what every graded run records.
ARG KRAKEN_BASE={image_ref}
FROM ${{KRAKEN_BASE}}
"""
...
class PerfRuntimePipeline:
    ...
    def emit_one(self, rec: dict, out_dir: Path, image_ref: str | None = None) -> _Emitted | None:
        ...
        if not image_ref:
            ...
            return None
        dockerfile = BUNDLE_DOCKERFILE.format(image_ref=image_ref)
